function I = PSLoadProcessedImagesMat(imgDir, imgSuffix, options)
...
imgFiles = dir(fullfile(imgDir, ['Image_*.' 'mat']));
nImgs = length(imgFiles);
I = load(fullfile(imgDir, imgFiles(1).name)).image;
[M, N, C] = size(I);

% Load images.
I = zeros(M, N, nImgs);
for i = 1:nImgs
  Itmp = im2double(load(fullfile(imgDir, imgFiles(i).name)).image);
